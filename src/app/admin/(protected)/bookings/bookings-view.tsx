...
async function getBookings(filters: {
  ...
  let query = supabase
    ...
    .order("check_in_date", { ascending: filters.mode === "current" });
...
function buildResetHref(mode: "current" | "archive") {
  return mode === "archive" ? "/admin/bookings/archive" : "/admin/bookings";
}

export async function AdminBookingsView({
  searchParams,
  mode,
}: AdminBookingsViewProps) {
  const { dateFrom, dateTo, status } = searchParams;
  ...
  return (
    <section className="space-y-8">
      <div>
        ...
        <h1 className="mt-3 text-4xl leading-tight text-brand-900">
          {isArchive ? "Archivio prenotazioni" : "Prenotazioni"}
        </h1>
        <p className="mt-3 max-w-3xl text-base leading-7 text-stone-600">
          {isArchive
            ? "Consulta le prenotazioni passate. Le righe restano nello stesso archivio dati e non vengono mai eliminate."
            : "Controlla le prenotazioni in corso o future, filtra elenco per stato o date del soggiorno e apri ogni scheda per vedere piu dettagli."}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <Link
          ...
          className={`inline-flex items-center justify-center rounded-full px-5 py-3 text-sm font-semibold transition ${
            isArchive
              ? "border border-stone-300 text-brand-900 hover:border-brand-500"
              : "bg-brand-700 text-white hover:bg-brand-800"
          }`}
        >
          Prenotazioni attive e future
        </Link>
        <Link
        ...
        {bookings.length === 0 ? (
          <div className="px-5 py-10 text-center sm:px-6">
            <p className="text-lg text-brand-900">
              Nessuna prenotazione corrisponde ai filtri attuali.
            </p>
